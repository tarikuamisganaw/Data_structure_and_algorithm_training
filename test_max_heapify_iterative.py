...
def test_max_heapify_sorted_array():
    ...
    # The largest element should be at the root
    assert arr[0] == 5  # Only check the root for max-heap property

def test_max_heapify_reverse_sorted_array():
    arr = [5, 4, 3, 2, 1]
    max_heapify(arr, len(arr), 0)
    # The largest element should still be at the root
    assert arr[0] == 5  # Only check the root for max-heap property
...
def test_max_heapify_unsorted_array():
    ...
    max_heapify(arr, len(arr), 0)
    # The largest element should be at the root
    assert arr[0] == 10  # Only check the root for max-heap property
